...
def generate_images_dict(imagelist,image_path,start_image_id=0):
	print('GENERATE_IMAGES_DICT...')
	images_dict=[]
	with tqdm(total=len(imagelist)) as load_bar:
		for x in imagelist:
			dict={IMAGES_DICT[0]:x,IMAGES_DICT[1]:load_image(osp.join(image_path,x))[0],\
					IMAGES_DICT[2]:load_image(osp.join(image_path,x))[1],IMAGES_DICT[3]:imagelist.index(x)+start_image_id}
			load_bar.update(1)
